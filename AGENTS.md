...
Clawd 桌宠 — 一个 Electron 桌面宠物，通过 hook 系统和日志轮询实时感知 AI coding agent 的工作状态并播放对应的像素风动画（SVG / APNG / GIF / PNG 等）。支持 **Codex**（command + HTTP hook）、**Codex CLI**（JSONL 日志轮询）、**Copilot CLI**（command hook）、**Cursor Agent**（`~/.cursor/hooks.json`，stdin JSON + stdout JSON）、**Gemini CLI**（session JSON 轮询）、**Kiro CLI**（per-agent `~/.kiro/agents/*.json`）、**CodeBuddy**（Codex-兼容 hook）、**opencode**（in-process plugin + 反向 HTTP bridge）并行运行。内置两套主题 **Clawd**（像素螃蟹）与 **Calico**（三花猫），并支持用户自定义主题。支持 Windows、macOS 和 Linux，UI 三语（en / zh / ko）。
...
npm run install:Codex-hooks   # 手动注册 Codex hooks 到 ~/.Codex/settings.json
npm run uninstall:Codex-hooks # 移除 Codex hooks
...
- `agents/codebuddy.js` — CodeBuddy 事件映射（PascalCase，Codex 兼容），支持权限
...
| `hooks/codebuddy-hook.js` + `codebuddy-install.js` | CodeBuddy hook（Codex 兼容 PascalCase）+ 注册到 ~/.codebuddy/settings.json |
...
| `hooks/install.js` + `uninstall.js` | Codex hooks 的安全注册 / 卸载（逐事件追加不覆盖）；`registerHooks()` 在 main.js 启动时自动调用 |
...
- CodeBuddy：Codex 兼容的 hook 格式，注册到 `~/.codebuddy/settings.json`；支持权限，capabilities 同 Codex 但无 subagent
