...
describe("settings renderer browser environment", () => {
  ...
  it("does not expose the removed About tab in the settings sidebar", () => {
    const source = fs.readFileSync(SETTINGS_RENDERER, "utf8");
    const sidebarStart = source.indexOf("const SIDEBAR_TABS = [");
    const sidebarEnd = source.indexOf("];", sidebarStart);
    assert.notStrictEqual(sidebarStart, -1, "settings-renderer.js should declare SIDEBAR_TABS");
    assert.notStrictEqual(sidebarEnd, -1, "SIDEBAR_TABS should be a closed array literal");

    const sidebarSource = source.slice(sidebarStart, sidebarEnd);
    assert.ok(!sidebarSource.includes('id: "about"'), "About should not appear in settings sidebar tabs");
    assert.ok(!source.includes('activeTab === "about"'), "About tab should not be directly renderable");
  });
});
